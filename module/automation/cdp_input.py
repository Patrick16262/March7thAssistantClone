...
class CdpInput(InputBase):
    ...
    def __init__(self, cloud_game: CloudGameController, logger):
    ...
    def focus(self):
        """当鼠标移出云游戏时，失去焦点时，键盘输入命令可能失效，这时候需要让浏览器判定鼠标还留在云游戏内"""
        try:
    ...
    # ---------------- Keyboard ----------------
    def press_key(self, key, wait_time=0.2, secretly=False):
        self.focus()
        k = key.lower()
    ...
    def secretly_press_key(self, key, wait_time=0.2):
        # 改为有效云游戏按键发送
        self.press_key(key, wait_time, secretly=True)
