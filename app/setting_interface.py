...
class SettingInterface(ScrollArea):
    ...
    def __initCard(self):
        ...
        self.cloudGameEnableCard = SwitchSettingCard1(
            FIF.SPEED_HIGH,
            self.tr('启用云游戏'),
            "开启后，将使用云游戏进行清体力等自动化任务。云游戏不依赖物理显示，支持后台运行。（模拟宇宙和锄大地除外）",
            "cloud_game_enable"
        )
        ...
        self.browserScaleCard = PushSettingCardEval(
            self.tr("修改"),
            FIF.ZOOM,
            self.tr("云游戏缩放比例（如果云游戏画面过小/过大，可适当增加/减少这个值）"),
        ...
        # self.browserUseRemoteCard = SwitchSettingCard1(
        #     FIF.DEVELOPER_TOOLS,
        #     self.tr("使用远程浏览器"),
        #     "",
        #     "browser_use_remote"
        ...
        # self.browserRemoteAddrCard = LineEditSettingCard(
        #     FIF.LINK,
        #     self.tr("远程浏览器地址"),
        #     self.tr("例如：http://localhost:4444/wd/hub"),
        #     "browser_remote_address"
        # )
